...
public class SignUpBOImpl {
    ...
    void txtPassOnKeyReleased(KeyEvent event) {
        Regex.setTextColor(com.example.ormcw.util.TextField.PASSWORD,txtPassword);
    }

    @FXML
    void txtRePassOnKeyReleased(KeyEvent event) {
        Regex.setTextColor(com.example.ormcw.util.TextField.PASSWORD,txtRePassword);
    }


    public int isValied(){
        if (!Regex.setTextColor(com.example.ormcw.util.TextField.NAME,txtUserName)) return 1;
        if (!Regex.setTextColor(com.example.ormcw.util.TextField.EMAIL,txtEmail)) return 2;
        if (!Regex.setTextColor(com.example.ormcw.util.TextField.PASSWORD,txtPassword)) return 3;
        if (!Regex.setTextColor(com.example.ormcw.util.TextField.PASSWORD,txtRePassword)) return 4;
        return 0;
    }
